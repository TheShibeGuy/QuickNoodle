...
namespace QuickNoodle
{
    ...
    {
        public Log log;
        public ArgumentEditor argEditor;
        public Form1()
        {
            InitializeComponent();
        ...
        private void Button1_Click(object sender, EventArgs e)
        {
            ...
            using (OpenFileDialog openFileDialog = new OpenFileDialog())
            {
                ...
                openFileDialog.Filter = "Beat Saber Map files (*.dat)|*.dat|JSON files (*.json)|*.json";
        ...
        private void parseBookmarks(dynamic bookmarks, dynamic map)
        {
            ...
            {
                progressBar1.Maximum = bookmarks.Count;
            ...
            foreach (dynamic bookmark in bookmarks)
            {
                ...
                /**/
                IEnumerator<ArgumentControl> ie = argEditor.Arguments.GetEnumerator();
                LinkedList<string> UserArgs = new LinkedList<string>();
                LinkedList<string> UserDataNames = new LinkedList<string>();
                LinkedList<DataType> UserDataType = new LinkedList<DataType>();
                while (ie.MoveNext())
                {
                    ArgumentControl control = ie.Current;
                    UserArgs.AddLast(control.ArgName);
                    UserDataNames.AddLast(control.DataName);
                    UserDataType.AddLast(control.DataType);
                ...
                foreach (string command in commands)
                {
                   ...
                   // welcome.Text = $"Processing {index}th bookmark";
                    ...
                    {
                        if (UserArgs.Contains(command))
                        {
                            int UserIndex = UserArgs.TakeWhile(n => n != command).Count();
            ...
            )
        {
                ...
                if (time <= _time /* if time of bookmark is less than time of note && _time < _nextTime*/)
                {
                    ...
                    lock (noteLock)
                    {
                        if (_note._type == 0)
                        {
                            if (noteReds[0] >= 0)
                            {
                                noteReds.Select(x => Math.Round(x, 4));
                        ...
                        else if (_note._type == 1)
                        {
                            if (noteBlues[0] >= 0)
                            {
                                noteBlues.Select(x => Math.Round(x, 4));
                    ...
                    lock (noteLock)
                    {
                        ...
                        if (worldRotation[0] == 0) { _note._customData.Remove("_rotation"); }
                        if (noteRotation[0] == 0) { _note._customData.Remove("_localRotation"); }
                        // Make sure this is always last
                        if (_note["_customData"].Count == 0) { _note.Remove("_customData"); }
                    }
            ...
            for ( int i = 0; i < mapCopy._events.Count; i++)
            {
                dynamic _event;
                dynamic prevEvent;
                JObject previousSpin = new JObject(
                    new JProperty("_type", 8),
                    new JProperty("_time", 0),
                    new JProperty("_customData", new JObject(
                        new JProperty("_direction", 0)
                        )
                    ));
                lock (eventLock)
                {
                    ...
                    lock (eventLock)
                    {
                            ...
                            case 8: // Ring rotation
                                ...
                                switch (direction)
                                {
                                    ...
                                    case 2:
                                        ...
                                        {
                                            if (_time - (float)previousSpin["_time"] < f)
                                            {
                                                if (previousSpin.ContainsKey("_customData"))
                                                {
                                                    if (previousSpin["_customData"]["_direction"] != null)
                                                    {
                        ...
                        switch (int.Parse(_event["_value"].ToString()))
                        {
                            ...
                            case 3:
                                if (eventBlues[0] >= 0)
                                {
                                    eventBlues.Select(x => Math.Round(x, 4));
                            ...
                            case 7:
                                if (eventReds[0] >= 0)
                                {
                                    eventReds.Select(x => Math.Round(x, 4));
                                ...
                                lock (balanceLock)
                                {
                                    // Console.WriteLine( _event["_customData"].HasValues.ToString() );
                                    if (_event.ContainsKey("_customData"))
                                    {
                                        if (_event["_customData"].HasValues == false)
                                        {
                                            _event.Remove("_customData");
                                        }
                                    }
            ...
            for ( int i = 0; i < mapCopy._obstacles.Count; i++)
            {
                ...
                if (time <= _time)
                {
                    lock (wallLock)
                    {
                        ...
                        wallColor.Select(x => Math.Round(x, 4));
                        if (wallColor[0] > 0) { Utilities.addOrUpdateElement(_wall._customData, "_color", wallColor, Utilities.JType.JArray);  }

                        lock (balanceLock)
                        {
                            if (_wall["_customData"].Count == 0) { _wall.Remove("_customData"); }
                            mapCopy._obstacles[i] = _wall;
                        }
                    }
        ...
        {
            System.Diagnostics.Process.Start("https://github.com/ShibeGuy/QuickNoodle/blob/master/README.md#help");
        }
        // Log
        ...
        {
            if (argEditor.Visible)
            {
                argEditor.Hide();
                argEditorButton.Text = "Open Arg Editor";
            }
            else
            {
                argEditor.Show();
                argEditorButton.Text = "Close Arg Editor";
            }
        }
